...
import com.xinlan.android.basesupport.util.ResourcesUtils;
...
public class ImageLoader {
    ...
    public static void loadImage(Object obj, int placeHolder,ImageView imageView) {
        if (obj instanceof String) {
            Glide.with(imageView.getContext()).load(obj).apply(initOptions())
    ...
    public static void loadImage(Object obj, @DrawableRes int placeHolder,@DrawableRes int error,ImageView imageView) {
        if (obj instanceof String) {
            Glide.with(imageView.getContext()).load(obj).apply(initOptions())
                    .skipMemoryCache(isSkipMemoryCache()).error(error)
                    .placeholder(placeHolder).into(imageView);
        }
        if (obj instanceof Bitmap) {
            Glide.with(imageView.getContext()).load(obj).apply(initOptions())
    ...
    public static void loadImageWithLoading(Object obj,@DrawableRes int error,ImageView imageView) {
        int placeHolder = R.drawable.common_ic_loading;
        if (obj instanceof String) {
            Glide.with(imageView.getContext()).load(obj).apply(initOptions())
                    .skipMemoryCache(isSkipMemoryCache()).error(error)
                    .thumbnail(Glide.with(imageView).load(placeHolder))
                    .listener(new RequestListener<Drawable>() {
                        @Override
                        public boolean onLoadFailed(@Nullable GlideException e, Object model, Target<Drawable> target, boolean isFirstResource) {
                            imageView.post(()-> imageView.setImageDrawable(ResourcesUtils.getDrawable(imageView.getContext(),error)));
                            return false;
                        }
    ...
    public static void loadImageAd(Context context, Object obj, ImageView imageView) {
        if (obj instanceof String) {
            Glide.with(context).load(obj).apply(initOptionsAd())
    ...
    public static void loadCircleImage(Context context, int resourceId, ImageView imageView) {
        Glide.with(context).load(resourceId).apply(initOptions())
                .error(getErrorImage()).placeholder(getPlaceholder())
                .fallback(getErrorImage()).circleCrop().into(imageView);
