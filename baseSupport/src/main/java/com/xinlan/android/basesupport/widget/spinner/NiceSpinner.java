...
public class NiceSpinner extends AppCompatTextView {
     ...
     * 显示多少个Item
     * */
    private int popupWindowItemNum = 5;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        popupWindowItemNum = typedArray.getInteger(R.styleable.NiceSpinner_popupWindowItemNum, popupWindowItemNum);
    ...
    private <T> void setAdapterInternal(NiceSpinnerBaseAdapter<T> adapter) {
        if (adapter.getCount() >= 0) {
            ...
            //这里是获取每个item大小用于判断是否需要缩减大小
            View item = adapter.getView(0,null,new ListView(getContext()));
            item.measure(0,0);
            int height = item.getMeasuredHeight();
            Log.e("提示","" + height);
            if (adapter.getCount() > popupWindowItemNum){
                popupWindow.setHeight(popupWindowItemNum * height);
            }
